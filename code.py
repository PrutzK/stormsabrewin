...
def astar(locs, edges, source, target):
    s = locs[source] #(x,y)
    t = locs[target]
    #create graph to run dijkstra on
    newEdges = {}
    for i in xrange(0,len(edges)): #iterate through locs
        u = locs[i]
        for j in edges[i]:
            v = locs[j]
            weight = euclid(u,v)
            if i in newEdges:
                newEdges[i] = newEdges[i] + [(j,weight)]
            else:
                newEdges[i] = [(j,weight)]
    path,distance = dijkstra(len(locs), newEdges, source, target)
